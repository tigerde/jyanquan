...
public class MechAccidentDao {
	private static final Logger log=Logger.getLogger(MechAccidentDao.class);
	...
	public static List<TrafficAccident> getTrafficAccidents(String starttime,String endtime,String sql){
		...
		try {
			...
			ps=conn.prepareStatement("select b.name,b.company,a.*  from mechanical_accident a left join drivers b on a.idcard=b.id where  a.deleted=0 and b.deleted=0 and   a.accident_date between ? and ? "+sql);
			...
			log.error("getTrafficAccidents获取驾驶员机械事故信息", e);
		}finally{
	...
	public static List<TrafficAccident> getTrafficAccidents(String id,String sql){
		...
		try {
			conn=DBConn.getConnection();
			ps=conn.prepareStatement("select * from mechanical_accident where  deleted=0 and idcard=? "+sql);
			ps.setString(1, id);
			rs=ps.executeQuery();
			while(rs.next()){
	...
	public static TrafficAccident getTrafficAccident(String id){
		...
		try {
			conn=DBConn.getConnection();
			ps=conn.prepareStatement("select * from mechanical_accident where deleted=0 and id=?");
			ps.setString(1, id);
			rs=ps.executeQuery();
			if(rs.next()){
			...
			log.error("getTrafficAccident获取驾驶员机械事故信息", e);
		}finally{
			DBConn.close(rs, ps, conn);
		}
		return ta;
	...
	public static int delTrafficAccident(String id){
		...
		try {
			conn=DBConn.getConnection();
			ps=conn.prepareStatement("update mechanical_accident set deleted=1 where id=? ");
			ps.setString(1, id);
			i=ps.executeUpdate();
			
		} catch (SQLException e) {
			log.error("delTrafficAccident删除驾驶员机械事故信息", e);
		}finally{
			DBConn.close(rs, ps, conn);
		}
		return i;
	...
	public static int updateTrafficAccident(TrafficAccident ta){
		...
		try {
			conn=DBConn.getConnection();
			ps=conn.prepareStatement("update mechanical_accident set company=?,line_number=?,bus_number=?,accident_date=?," +
	...
	public static int insertTrafficAccident(TrafficAccident ta){
		...
		try {
			conn=DBConn.getConnection();
			ps=conn.prepareStatement("insert into  mechanical_accident (company,line_number,bus_number,accident_date," +
	...
	public static int getTrafficAccidentCount(String id){
		...
		conn=DBConn.getConnection();
		try {
			ps=conn.prepareStatement("select count(*) count from mechanical_accident where idcard=? and deleted=0");
			ps.setString(1, id);
			rs=ps.executeQuery();
			if(rs.next()){
				i=rs.getInt("count");
			...
			log.error("getTrafficAccidentCount驾驶员机械事故列表总数", e);
		}finally{
			DBConn.close(rs, ps, conn);
		}
	...
	public static int getTrafficAccidentCount(String starttime,String endtime){
		...
		conn=DBConn.getConnection();
		try {
			ps=conn.prepareStatement("select count(*) count from mechanical_accident a left join drivers b on a.idcard=b.id where  a.deleted=0 and b.deleted=0 and   a.accident_date between ? and ?");
			ps.setString(1, starttime);
			ps.setString(2, endtime);
			rs=ps.executeQuery();
			if(rs.next()){
