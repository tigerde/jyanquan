...
public class SafetyDistanceDao {
	private static final Logger log=Logger.getLogger(SafetyDistanceDao.class);
	...
	public static int delSafetyDistance(String id){
		...
		try {
			...
			ps.setString(1, id);
			i=ps.executeUpdate();
			ps=conn.prepareStatement("select * from  safety_distance where id=? ");
			ps.setString(1, id);
			rs=ps.executeQuery();
			if(rs.next()){
			//ps=conn.prepareStatement("update safety_distance set mileage_year=(  select sum(mileage_month)   from safety_distance where year =? and idcard=? and deleted=0)"+
			//",mileage=(select sum(mileage_month)   from safety_distance where idcard=? and deleted=0),modifi_datetime=?,modifi_user_id=? where idcard=? and deleted=0");
				ps=conn.prepareStatement("update safety_distance set mileage_year=(  select sum(mileage_month)   from safety_distance where year =? and idcard=? and deleted=0)"+
				",mileage=(select sum(mileage_month)   from safety_distance where idcard=? and deleted=0)where idcard=? and deleted=0");
				ps.setInt(2, rs.getInt("year"));
	...
	public static int updateSafetyDistance(SafetyDistance be){
		...
		try {
			conn=DBConn.getConnection();
			ps=conn.prepareStatement("update safety_distance set bus_number=?,year=?,month=?,mileage_month=?," +
			...
			ps=conn.prepareStatement("update safety_distance set mileage_month=0,mileage_year=(  select sum(mileage_month)   from safety_distance where year =? and idcard=? and deleted=0)"+
			",mileage=(select sum(mileage_month)   from safety_distance where idcard=? and deleted=0),modifi_datetime=?,modifi_user_id=? where idcard=? and deleted=0");
			ps.setInt(1, be.getYear());
			ps.setString(2, be.getIdcard());
			ps.setString(3, be.getIdcard());
			ps.setString(4, be.getModifi_datetime());
			ps.setString(5, be.getModifi_user_id());
			ps.setString(6, be.getIdcard());
			i=ps.executeUpdate();
	...
	public static int insertSafetyDistance(SafetyDistance be){
		...
		try {
			conn=DBConn.getConnection();
			ps=conn.prepareStatement("insert into  safety_distance (bus_number,year,month,mileage_month," +
			...
			/*
			ps=conn.prepareStatement("update safety_distance set mileage_month=0,mileage_year=(  select sum(mileage_month)   from safety_distance where year =? and idcard=? and month!=0 and deleted=0)"+
			",mileage=(select sum(mileage_month)   from safety_distance where idcard=?  and month!=0  and deleted=0),modifi_datetime=?,modifi_user_id=? where idcard=? and year=? and month=0 and deleted=0");
			ps.setInt(1, be.getYear());
			ps.setString(2, be.getIdcard());
			ps.setString(3, be.getIdcard());
			ps.setString(4, be.getModifi_datetime());
			...
			/*if(i==0){
				...
				"modifi_datetime,modifi_user_id,idcard,create_datetime,create_user_id,deleted) values(?,?,?,0," +
				" (select sum(mileage_month)   from safety_distance where year =? and idcard=?  and month!=0  and deleted=0)," +
				"(select sum(mileage_month)   from safety_distance where idcard=? and  month!=0  and deleted=0),?,?,?,?,?,0) ");
				ps.setString(1, be.getBus_number());
				ps.setInt(2, be.getYear());
				ps.setInt(3, be.getMonth());
				//ps.setDouble(4, be.getMileage_month());
				//ps.setDouble(5, be.getMileage_year());
				//ps.setDouble(6, be.getMileage());
				ps.setInt(4, be.getYear());
				ps.setString(5, be.getIdcard());
				ps.setString(6, be.getIdcard());
